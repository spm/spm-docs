...
The data provided ([here](https://www.fil.ion.ucl.ac.uk/spm/data/eeg_supramodal/)) for this tutorial have already been preprocessed. However, there are still a few preparation steps required: we need to re-reference, filter, and mark artifacts. To do so:
...
<figure markdown>
  ![Options for the Prepare batch](../../assets/figures/tutorials/meeg/meeg_firstlevel/prepare-options.png){#fig:meeg-firstlevel:prepare}
...
<figure markdown>
  ![Options for the Filter batch](../../assets/figures/tutorials/meeg/meeg_firstlevel/filter-options.png){#fig:meeg-firstlevel:filter}
...
<figure markdown>
  ![Options for the Artefacts batch](../../assets/figures/tutorials/meeg/meeg_firstlevel/artefacts-options.png){#fig:meeg-firstlevel:artefacts-options}
  <figcaption>Options for the Artefacts batch</figcaption>
</figure>

-   Run the batch with the "Play" button.
...
-   Load the file called "events.mat". The events onset times are stored in 3 cells corresponding to visual, auditory, and tactile events.

-   On the main SPM window, click on "Specify 1st-level"
...
-   Specify the timing parameters
    ...
    -   Under "Time window", set $[-300 \;\; 700]$. This will model the response between 300ms before and 700ms after event onset.

    -   Under "Unit for design", set "seconds"

<figure markdown>
  ![Setting the channels and timing parameters of the convolution model](../../assets/figures/tutorials/meeg/meeg_firstlevel/conv-top.png){#fig:meeg-firstlevel:conv-top}
...
<figure markdown>
  ![Specifying the conditions of the convolution model](../../assets/figures/tutorials/meeg/meeg_firstlevel/conv-other-conds.png){#fig:meeg-firstlevel:conv-other}
...
<figure markdown>
  ![Setting the basis set of the convolution model](../../assets/figures/tutorials/meeg/meeg_firstlevel/conv-basis-set.png){#fig:meeg-firstlevel:conv-basis}
  <figcaption>Specifying the conditions of the convolution model</figcaption>
</figure>
...
<figure markdown>
  ![Final prepare batch to fix sensors types and locations.](../../assets/figures/tutorials/meeg/meeg_firstlevel/prepare-post-conv.png){#fig:meeg-firstlevel:prepare-post-conv}
...
-   First, it creates a design matrix $X$ for our problem $Y = X \beta + \varepsilon$, with time as rows and basis functions for each condition as columns.
...
-   Lastly, it uses the basis function weights for condition $c$ to construct the deconvolved responses, represented by $y_c = x_{BF} \beta_c$.
...
<figure markdown>
  [Scalp maps of deconvolved responses to the different stimuli at particular time of interest](../../assets/figures/tutorials/meeg/meeg_firstlevel/scalp.png){#fig:meeg-firstlevel:scalp}
  <figcaption>Scalp maps of deconvolved responses to the different stimuli at particular time of interest.</figcaption>
</figure>

<figure markdown>
  ![Average responses over fronto-central channels for the different stimuli](../../assets/figures/tutorials/meeg/meeg_firstlevel/result-response.png){#fig:meeg-firstlevel:responses}
